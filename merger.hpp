...
void merger(data_stream* from2, data_stream* from3,
        data_stream* from4, data_stream* from5, data_stream* from6, data_stream* from7, data_stream* to1){
...
#pragma HLS PIPELINE II=1
		...
		if ((have_read3 || from2->empty() || !partial_end3) && (have_read4 || from3->empty() || !partial_end4) &&
				...
				&& (have_read7 || from6->empty() || !partial_end7) && (have_read8 || from7->empty() || !partial_end8)){
			have_read3 = false;
			have_read4 = false;
			have_read5 = false;
			have_read6 = false;
		...
		whether_read7 = !have_read7 && !from6->empty() && partial_end7;
		whether_read8 = !have_read8 && !from7->empty() && partial_end8;
		...
		else if (whether_read7)  {
			val_data<val_t> temp = from6->read();
			final_end7=temp.final_end;
			partial_end7 = temp.partial_end;
			if (temp.final_end){
				if (temp.partial_end){
					to1->write(temp);
				} else {
		...
		else if (whether_read8)  {
			val_data<val_t> temp = from7->read();
			final_end8=temp.final_end;
			partial_end8 = temp.partial_end;
			if (temp.final_end){
				if (temp.partial_end){
					to1->write(temp);
				} else {
		...
		enable = final_end3 || final_end4 || final_end5 || final_end6 || final_end7 || final_end8;
	}
